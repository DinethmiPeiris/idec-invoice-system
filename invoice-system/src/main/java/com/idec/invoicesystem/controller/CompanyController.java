...
public class CompanyController {
    ...
    @PostMapping("/edit/{id}")
    public String editCompany(@PathVariable String id, @ModelAttribute Company company, RedirectAttributes ra) {
        java.util.Optional<Company> existingOpt = companyService.getCompanyById(id);
        if (existingOpt.isEmpty()) {
            ra.addFlashAttribute("error", "Consignee not found.");
            return "redirect:/companies";
        }
        Company existing = existingOpt.get();
        if (!existing.getName().equalsIgnoreCase(company.getName()) && companyService.existsByName(company.getName())) {
            ra.addFlashAttribute("error", "Company '" + company.getName() + "' already exists.");
            return "redirect:/companies";
        }
        company.setId(id);
        company.setActive(existing.isActive());
        companyService.saveCompany(company);
        ra.addFlashAttribute("success", "Consignee updated successfully.");
        return "redirect:/companies";
    }
